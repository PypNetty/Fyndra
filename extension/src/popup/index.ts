import { Alert } from "@jobscan/shared";
import "./styles.scss";
    ...
    chrome.runtime.sendMessage(
      ...
      (response: { error?: string; tabId?: number; url?: string }) => {
        ...
        if (response.error || !response.tabId || !response.url) {
          showError(
            response.error || "Impossible de récupérer l'onglet actif."
          );
          return;
        }
...
function checkForExistingAnalysis() {
  chrome.storage.local.get(
    ...
    (data: { currentAnalysis?: { url: string | null; alerts?: Alert[] } }) => {
      if (chrome.runtime.lastError) {
        console.error(
          `Erreur storage.local.get: ${chrome.runtime.lastError.message}`
        );
        return;
      }
...
function displayAnalysisResults(jobOffer: {
  ...
  shareBtn.onclick = async () => {
    ...
    if (!token) {
      alert(
        "Vous devez être connecté à JobScan pour partager une offre sur votre dashboard."
      );
      return;
    }
    if (!email) {
      email = prompt("Entrez votre email JobScan pour partager cette offre :");
    }
    if (!email) return;
    try {
