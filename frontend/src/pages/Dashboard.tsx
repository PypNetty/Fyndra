...
const Dashboard: React.FC = () => {
  ...
  const [filter, setFilter] = useState<"all" | "rejected" | "shared">("all");
  ...
  const ApplyModal = ({ offer, onClose }: { offer: JobOffer, onClose: () => void }) => {
    ...
    useEffect(() => {
      ...
      const missing = (profile.technos || []).length > 0 ?
        (profile.technos as string[]).filter(tech => !offerText.includes(tech.toLowerCase())) : [];
          ...
          {step === 1 ? (
            <>
              ...
              {missingTechs.length > 0 && (
                <div className="mb-4">
                  <label className="block font-medium mb-1">Technos non présentes dans votre profil :</label>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-blue-100 to-blue-200 flex">
      ...
      <div className="flex-1 flex flex-col">
        {/* Utilisation du composant TopNav au lieu du header */}
        <TopNav />
                ...
                return (
                  <div key={offer.id} className="bg-white rounded-xl shadow p-6 flex flex-col gap-2 items-start border-l-4 border-blue-200">
                    ...
                    <div className="flex gap-2 mt-2">
                      ...
                      {offer.status !== "rejected" && (
                        <button className="flex items-center gap-1 text-xs bg-green-100 text-green-700 px-2 py-1 rounded" onClick={() => { setOfferToApply(offer); setShowApplyModal(true); }}>
                    ...
                    {offer.status === "rejected" && <span className="mt-2 text-xs text-red-500 flex items-center gap-1"><XCircleIcon className="h-4 w-4" /> Rejetée</span>}
                    {offer.status === "shared" && <span className="mt-2 text-xs text-blue-500 flex items-center gap-1"><ShareIcon className="h-4 w-4" /> À partager</span>}
                  </div>
                );
              })}
            </div>
        ...
        {showApplyModal && offerToApply && (
          <ApplyModal offer={offerToApply} onClose={() => { setShowApplyModal(false); setOfferToApply(null); }} />
        )}
